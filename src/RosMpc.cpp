...
RosMpc::RosMpc(ros::NodeHandle *nh)
    : controlSys_{nh->param<double>("path_tracking_vel", 0.0),
                  nh->param<double>("parking_vel", 0.0)},
      tfListener_{tfBuffer_},
      nh_{nh} {
    if (!verifyParamsForMPC(nh)) {
        ROS_WARN(
            "One or more parameters for the mpc is not specified. Default values is therefore "
            "used.");
    }
    ...
    std::string actualSteeringTopic =
        nh_->param<std::string>("actual_steering_topic", "actual_steering_angle");
    ...
    if (modeStr == "parking") {
        ...
        std::stringstream ss;
        ss << "Invalid mode for mpc: " << modeStr
           << ". Valid modes are parking, slalom and path_tracking";
    ...
    actualSteeringSub_ =
        nh->subscribe(actualSteeringTopic, 1, &RosMpc::actualSteeringCallback, this);
...
MPCReturn RosMpc::solve() {
    ...
    try {
        ...
        ROS_ERROR_STREAM("Could not get transform from " << mapFrame_ << " to " << carFrame_
                                                         << ". Error thrown: " << e.what());
        return MPCReturn{};
    }
...
bool RosMpc::verifyInputs() {
    ...
    std::string twistTopic = nh_->param<std::string>("twist_topic", "/twist");
    while (ros::ok() &&
           !ros::topic::waitForMessage<geometry_msgs::TwistStamped>(twistTopic, waitTime)) {
        ROS_WARN("Waiting for twist message. Should be published at the topic: %s",
                 twistTopic.c_str());
    }
    ...
    std::string actualSteeringTopic =
        nh_->param<std::string>("actual_steering_topic", "/actual_steering_angle");
    while (ros::ok() &&
           !ros::topic::waitForMessage<std_msgs::Float64>(actualSteeringTopic, waitTime)) {
        ROS_WARN("Waiting for actual steering angle. Should be published at the topic: %s",
                 actualSteeringTopic.c_str());
    }
    ...
    if (nh_->hasParam("path_topic") && mode_ == Mode::PathTracking) {
        std::string pathTopic = nh_->param<std::string>("path_topic", "/path");
        while (ros::ok()) {
            nav_msgs::Path::ConstPtr firstPath =
                ros::topic::waitForMessage<nav_msgs::Path>(pathTopic, waitTime);
            if (firstPath != nullptr) {
                controlSys_.setTrack(util::toVector(*firstPath));
                break;
            }
            ROS_WARN("Waiting for path message. Should be published at the topic: %s",
                     pathTopic.c_str());
        }
    }

    if (mode_ == Mode::Parking) {
        std::string parkingTopic =
            nh_->param<std::string>("parking_topic", "move_base_simple/goal");
        while (ros::ok()) {
            geometry_msgs::PoseStamped::ConstPtr firstPose =
                ros::topic::waitForMessage<geometry_msgs::PoseStamped>(parkingTopic, waitTime);
            if (firstPose != nullptr) {
                controlSys_.setRefPose(firstPose->pose);
                break;
            }
            ROS_WARN(
                "Waiting for pose message for parking spot. Should be published at the topic: %s",
                parkingTopic.c_str());
        }
    }
    ...
    while (ros::ok()) {
        ...
        try {
            ...
            ROS_WARN("Waiting for transfrom from map: \"%s\" to car: \"%s\".\nError message: %s",
                     mapFrame_.c_str(), carFrame_.c_str(), e.what());
        }
    }
    return true;
}
...
void RosMpc::pathCallback(const nav_msgs::Path::ConstPtr &msg) {
    ...
    if (msg->header.frame_id != mapFrame_) {
        ROS_WARN_STREAM_THROTTLE(
            20, "Path has different frame than the MPC use. Therefore a transform will be done.");
        geometry_msgs::TransformStamped tfStampedPathFrame;
        try {
            tfStampedPathFrame =
                tfBuffer_.lookupTransform(mapFrame_, path.header.frame_id, ros::Time(0));
        } catch (tf2::TransformException &e) {
            ROS_ERROR_STREAM("Could not get transform from "
                             << mapFrame_ << " to " << path.header.frame_id
                             << ". Error thrown: " << e.what()
        ...
        for (auto &stampedPose : path.poses) {
            ...
            tfOriginal.setRotation(tf2::Quaternion{pose.orientation.x, pose.orientation.y,
...
void RosMpc::poseCallback(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    ...
    if (msg->header.frame_id != mapFrame_) {
        ...
        tfGoal.setRotation(tf2::Quaternion{pose.orientation.x, pose.orientation.y,
                                           pose.orientation.z, pose.orientation.w});
        ...
        try {
            ...
            ROS_ERROR_STREAM(
                ...
                << mapFrame_ << " to " << msg->header.frame_id << ". Error thrown: " << e.what()
